...
def resource(uri):
    ...
    handler = {
           "PUT": lambda: rest_service.create(..., record),
           "GET": lambda: rest_service.read(uri),
          "HEAD": lambda: rest_service.read(uri),  # eg. curl -I issues a HEAD request
         "PATCH": lambda: rest_service.update(..., record),
        "DELETE": lambda: rest_service.delete(..., id)}
